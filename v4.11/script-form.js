...
jQuery(document).ready(function () {
    ...
    function handleTextareaInput(value, paramKey, paramOption, $checkbox) {
        // Update metrics
        if (window.metrics) {
    ...
    function handleCustomTextareaInput(value, paramKey, paramOption, $checkbox) {
    ...
    function toggleCustomTextarea($modal, paramKey, $select, $customTextarea) {
    ...
    function attachParameterEventHandlers($modal, paramKey, paramOption, sectionKey) {
        ...
        if ($textarea.length) {
            $textarea.on('input', function() {
                handleTextareaInput($(this).val(), paramKey, paramOption, $checkbox);
            });
        }
        ...
        if ($customTextarea.length) {
            $customTextarea.on('input', function() {
                handleCustomTextareaInput($(this).val(), paramKey, paramOption, $checkbox);
            });
        }
        ...
        if ($editButton.length && $select.length && $customTextarea.length) {
            $editButton.on('click', function() {
                toggleCustomTextarea($modal, paramKey, $select, $customTextarea);
            });
        }
    }
