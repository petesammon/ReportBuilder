...
/* EchoTools v6.20 - Form Building and ContentEditable Handlers */
...
jQuery(document).ready(function () {
    ...
    function setupContentEditableHandlers() {
        ...
        console.log('[v6.20 Form] ContentEditable handlers attached');
    }
    ...
    function buildForm() {
        ...
        console.log('[v6.20 Form] Form built');
    }
    ...
    /**
     * Get visible modal keys in order
     */
    ...
    /**
     * Get previous modal key
     */
    function getPreviousModalKey(currentModalKey) {
        const keys = getVisibleModalKeys();
    ...
    /**
     * Check if this is the first modal
     */
    function isFirstModal(modalKey) {
        const keys = getVisibleModalKeys();
        return keys.indexOf(modalKey) === 0;
    }
     ...
     * Back button is hidden (not disabled) on first modal
    ...
    function buildModal(modalKey, section) {
        ...
        // Back button (invisible placeholder if first modal to maintain layout)
        if (isFirst) {
        ...
        $modal.find('.modal-back-button').on('click', function() {
            saveModalChanges($modal, section);
            $modal.removeClass('active');
        ...
        $modal.find('.modal-next-button').on('click', function() {
            saveModalChanges($modal, section);
            $modal.removeClass('active');
    ...
    function buildModalMeasurementRow(measurementKey) {
        ...
        $valueWrapper.append(`
            <input type="text" class="modal-measurement-input${calculatedClass}" data-measurement="${measurementKey}" value="${value}" ${isCalculated ? 'title="Calculated field"' : ''}>
    ...
    function updateModalContent($modal, section) {
        ...
        const nextKey = getNextModalKey(modalKey);
        $modal.find('.modal-next-button').prop('disabled', !nextKey);
