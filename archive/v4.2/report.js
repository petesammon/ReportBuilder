...
jQuery(document).ready(function () {
    ...
    function updateSectionPreview(sectionKey) {
        ...
        try {
            ...
            const $preview = $(`#${sectionKey}-preview`);
            $preview.val(rendered);
    ...
    ` : ''}
                ...
                if (option.clickText && option.custom) {
                    ...
                    // Also update section preview when textarea content changes
                ...
                $("select, textarea", $option).on("change", function () {
                    ...
                    // Update section preview if this field belongs to a section with preview
                    if (section.enableSectionPreview && section.sectionPreviewKey) {
                        updateSectionPreview(section.sectionPreviewKey);
                    }
            ...
            if (section.enableSectionPreview && section.sectionPreviewKey) {
                ...
                // Create modal for section options
                const modalId = `${previewKey}-options-modal`;
                const $modal = $(`<div id="${modalId}" class="modal">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h2>${section.title} Options</h2>
                            <button class="close-button section-modal-close">&times;</button>
                        </div>
                        <div class="modal-body">
                            <div id="${previewKey}-options-container"></div>
                            <div style="text-align: center; margin-top: 1.5rem;">
                                <button type="button" id="${previewKey}-generate" class="generate-section-button">Generate Text</button>
                            </div>
                        </div>
                    </div>
                </div>`);
                ...
                // Move all parameter fields for this section into the modal
                const $optionsContainer = $(`#${previewKey}-options-container`);
                Object.entries(section.params).forEach(([paramKey, paramOption]) => {
                    const $paramField = $(`#${paramKey}`);
                    if ($paramField.length) {
                        $paramField.detach().appendTo($optionsContainer);
                    }
                });
                
                // Create section preview display
                const $sectionPreview = $(`<div class="section-preview">
                    <div class="button-container">
                        <button type="button" id="${previewKey}-options-btn" class="options-button" title="Edit section options">Options</button>
                    </div>
                ...
                $(`#${previewKey}-options-btn`).on("click", function() {
                    $(`#${modalId}`).addClass("active");
                });
                
                // Close modal handlers
                $(`#${modalId} .section-modal-close`).on("click", function() {
                ...
                // Generate text button updates preview
