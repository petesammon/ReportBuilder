...
jQuery(document).ready(function () {
    ...
    // Register Handlebars helper for processing nested templates (measurement strings)
    Handlebars.registerHelper('processTemplate', function(templateString) {
        if (!templateString) return '';
        try {
            const template = Handlebars.compile(templateString);
            return new Handlebars.SafeString(template(this));
        } catch (e) {
            console.warn('Failed to process template:', templateString, e);
    ...
    function populateSectionTextareas() {
        // First, collect default values from options configuration
        options.forEach(section => {
            if (section.params) {
                Object.entries(section.params).forEach(([key, option]) => {
                    // Find default option
        ...
        // Now render and populate each section textarea
        options.forEach(section => {
            if (section.enableSectionPreview && section.sectionPreviewKey) {
