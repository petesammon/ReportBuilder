...
jQuery(document).ready(function () {
    ...
    function createSummaryModal(section) {
        ...
        Object.entries(section.params).forEach(([paramKey, paramOption]) => {
            ...
            // Textarea change handler (for custom fields)
            if ($textarea.length) {
                $textarea.on('input', function() {
                    const textValue = $(this).val();
                    
                    if (window.metrics) {
                        window.metrics[paramKey] = textValue;
                    }
                    
                    // Auto-check/uncheck for summary checkboxes
                    if (paramOption.enableSummary && $checkbox.length && paramOption.summaryOnChange === true) {
                        // Only auto-check if summaryOnChange is true
                        const shouldCheck = textValue.trim() !== '';
