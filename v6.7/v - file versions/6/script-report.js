...
/* EchoTools v6.7 - Central Registry with Span-based ContentEditable + Algorithms + Multiple Modal Flows + Dynamic Options File Buttons */
...
jQuery(document).ready(function () {
    ...
    function postProcessReport(reportHTML, data) {
        ...
        if (window.options) {
            window.options.forEach(modalGroup => {
                ...
                const buttonHTML = `<button type="button" class="inline-edit-button" data-modal="${modalKey}" title="Edit ${modalGroup.modalTitle || modalKey}"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4 L4 4 C2.9 4 2 4.9 2 6 L2 20 C2 21.1 2.9 22 4 22 L18 22 C19.1 22 20 21.1 20 20 L20 13"></path><path d="M18.5 2.5 C19.3 1.7 20.7 1.7 21.5 2.5 C22.3 3.3 22.3 4.7 21.5 5.5 L12 15 L8 16 L9 12 Z"></path></svg></button>`;
    ...
    function updateSummary() {
        ...
        // v6.7: Clean up orphaned line breaks (lines with no visible content)
        cleanupOrphanedLineBreaks($textarea);
    ...
    /**
     * Remove line break spans that precede empty lines (v6.7)
     */
    function cleanupOrphanedLineBreaks($textarea) {
        const $lineBreaks = $textarea.find('[data-type="summary-linebreak"]');
        
        $lineBreaks.each(function() {
            const $lb = $(this);
            const lineNum = parseInt($lb.attr('data-line')) || 0;
            
            // Check if there are any non-empty summarystring spans on this line
            const $lineSpans = $textarea.find(`[data-type="summarystring"][data-line="${lineNum}"]`);
            let hasContent = false;
            
            $lineSpans.each(function() {
                if ($(this).text().trim()) {
                    hasContent = true;
                    return false; // break
                }
            ...
            // If no content on this line, remove the line break
            if (!hasContent) {
                const handle = $lb.attr('data-handle');
                if (handle && registry[handle]) {
    ...
    /**
     * Create dynamic buttons for each options file
     * Buttons are inserted between template dropdown and auto/reset buttons
     */
    function createOptionsFileButtons() {
        const $container = $('#options-file-buttons');
        if (!$container.length) {
            console.warn('[v6.7] Options file buttons container not found');
            return;
        }
        ...
        // Create a button for each options file
        optionsFiles.forEach((fileEntry, index) => {
            const defaultIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            const icon = (fileEntry.meta && fileEntry.meta.icon) ? fileEntry.meta.icon : defaultIcon;
            const title = (fileEntry.meta && fileEntry.meta.title) ? fileEntry.meta.title : `Open options file ${index + 1}`;
            
            const $button = $(`<button class="icon-button options-file-button" data-file-index="${fileEntry.fileIndex}" title="${title}">${icon}</button>`);
            $container.append($button);
        });
        
        console.log(`[v6.7] Created ${optionsFiles.length} options file button(s)`);
    }
    ...
    function loadReportConfig(configId, forceFullRebuild = false) {
        ...
        if (!needsFullRebuild) {
            ...
            // Clear previous options files tracking for partial update
            optionsFiles = [];
    ...
    // Options file buttons - opens first modal in corresponding options file
    // Uses event delegation for dynamically created buttons
    $(document).on("click", ".options-file-button", function() {
        const fileIndex = parseInt($(this).attr('data-file-index'), 10);
        const firstModalInFile = window.getFirstModalInFile(fileIndex);
        if (firstModalInFile && typeof window.openModal === 'function') {
            window.openModal(firstModalInFile);
        } else {
            console.warn(`[v6.7] No modals found in options file ${fileIndex}`);
        }
    });
    ...
    window.createOptionsFileButtons = createOptionsFileButtons;
