...
jQuery(document).ready(function () {
    ...
    function clearSpanContent($span) {
        ...
        if (type === 'measurement') {
            ...
            // v6.7: Clear summarystring and uncheck the linked parameter
            const entry = window.getRegistryEntry(handle);
            if (entry) {
                entry.summarystring = '';
            }
    ...
    function handleEnterKey(range, selection) {
        ...
        if ($span && $span.length) {
            ...
            // v6.7: For summarystring and summary-linebreak spans, break OUT of the span
            // instead of inserting <br> inside
            if (type === 'summarystring' || type === 'summary-linebreak') {
                const br = document.createElement('br');
                
                // Insert <br> after the span
                if ($span[0].nextSibling) {
                    $span[0].parentNode.insertBefore(br, $span[0].nextSibling);
                } else {
                    $span[0].parentNode.appendChild(br);
                }
                
                // Create a new manual edit span after the <br>
                ...
                // Position cursor in the new manual span
                const newRange = document.createRange();
