...
/* EchoTools v6.7 - Form Building and ContentEditable Handlers + Summarystring Support */
...
jQuery(document).ready(function () {
    ...
    function setupContentEditableHandlers() {
        ...
        console.log('[v6.7 Form] ContentEditable handlers attached');
    }
    
    /**
     * Handle input event - update registry without moving cursor
     * v6.7: Added support for summarystring spans
     */
    function handleInputEvent(e) {
        ...
        if ($span && $span.length) {
            ...
            // v6.7: Handle summarystring spans
            if (type === 'summarystring') {
                // Update summarystring in registry
                if (currentContent !== entry.summarystring) {
                    window.updateSummaryStringFromEdit($span, currentContent);
                }
                return;
            }
            ...
            if (currentContent !== expectedValue) {
                ...
                $span.removeClass('span-clean-measurement span-clean-parameter span-clean-fixed span-clean-summarystring span-unknown');
    ...
    /**
     * Clear span content - v6.7: Added summarystring support
     */
    function clearSpanContent($span) {
        ...
        if (type === 'measurement') {
        ...
        } else if (type === 'summarystring') {
            // v6.7: Clear summarystring - uncheck the linked parameter
            const entry = window.getRegistryEntry(handle);
            if (entry) {
                entry.summarystring = '';
            }
            ...
            const $checkbox = $(`.summary-checkbox[data-param="${handle}"], .modal-summary-checkbox[data-param="${handle}"]`);
            if ($checkbox.length) {
                $checkbox.prop('checked', false);
            }
        } else {
        ...
        if (type === 'parameter' || type === 'summarystring') {
    ...
    function handleEnterKey(range, selection) {
        ...
        if ($span && $span.length) {
            ...
            if (type === 'measurement') {
                ...
                // v6.7: Update summarystring on enter
                window.updateSummaryStringFromEdit($span, newContent);
            } else {
    ...
    /**
     * Handle paste event - v6.7: Added summarystring support
     */
    function handlePaste(e) {
        ...
        if ($span && $span.length) {
            ...
            if (type === 'measurement') {
                ...
                // v6.7: Update summarystring on paste
                window.updateSummaryStringFromEdit($span, newContent);
            } else {
                window.setRegistryValue(handle, newContent, 'dirty');
            }
    ...
    // ============================================================================
    // CONTEXT MENU - Enhanced for all span types (v6.7: Added summarystring)
    // ============================================================================
    
    /**
     * Handle context menu for any span type
     * - Parameters: show options list with (custom text) at top
     * - Measurements/Fixed/Manual/Summarystring: show (custom text) only
     */
        ...
        $menu.on('click', '.context-menu-custom', function() {
            ...
            if (entry) {
                if (type === 'measurement') {
                    ...
                    // v6.7: Mark summarystring as dirty
                    entry.status = 'dirty';
                } else {
    ...
    function buildForm() {
        ...
        console.log('[v6.7 Form] Form built');
    }
    ...
    function openModal(modalKey) {
        const section = window.options?.find(s => s.modalKey === modalKey);
        if (!section) {
            console.warn(`[v6.7 Modal] Section not found for modalKey: ${modalKey}`);
            return;
        }
    ...
    function buildModal(modalKey, section) {
        ...
        // Debug logging
        console.log(`[v6.7 Modal] Building ${modalKey}: fileIndex=${fileIndex}, isFirst=${isFirst}, isLast=${isLast}, prevKey=${prevKey}, nextKey=${nextKey}`);
        ...
        $modal.find('.modal-summary-back').on('click', function() {
            ...
            console.log('[v6.7 Modal] Summary back clicked, going to:', backModal, '(lastModalBeforeSummary was:', lastModalBeforeSummary, ')');
            if (backModal) {
                openModal(backModal);
            } else {
                console.warn('[v6.7 Modal] No back modal found for Summary');
            }
        });
